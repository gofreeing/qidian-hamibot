...
function clickParentIfClickable(widget) {
    ...
    if (widget === null) {
        console.log('找不到');
        return clickResults = false;  // 终止递归的条件：如果 widget 是空值，则结束递归
    }
    if (widget.click()) {
        console.log('已点击');
        return clickResults = true;  // 点击控件
    }
...
function lookvd() {
    ...
    while (text("看视频开宝箱").exists()) {
        ...
        waitad()
        clickParentIfClickable(text("我知道了").findOne(1500))
    }
